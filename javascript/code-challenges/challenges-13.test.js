...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(v => v.replaceAll(/[() -]/g, '') );
};
...
const findAnything = (arr, target) => {
  // Solution code here...
  return arr.filter(v => v.includes(target));
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(v => v.filter(val => !val.includes('Brook')));
};
...
const sortByDay = (arr) => {
  // Solution code here...
  return daysOfWeek.map(v => arr.filter(val => val.includes(v)));
};
