...
const count = (target, input) => {
  // Solution code here...
  return input.flat(1).reduce((acc, v) => v === target ? acc + 1 : acc, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat(1).reduce((acc, v) => acc + v, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(v => v
    .filter(v => typeof v === 'number' && v % 5 === 0)
    .map(v => Math.pow(2, v))
  );
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((acc, v) => parseInt(v.height) < parseInt(acc.height) ? v : acc).name;
};
