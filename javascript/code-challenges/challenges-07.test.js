...
const listFoods = (recipe) => {
  ...
  for (let i of recipe.ingredients) {
    // result.push(i.replace(/\d+\s\w+(-\w+)?\s/, ''));
...
const stepActions = (recipe) => {
  ...
  for (let i of recipe.steps) {
    result.push(i.slice(0, i.indexOf(' ')));
  }
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  evens.forEach(v => arr.splice(v, 1));
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  // Solution code here...
  const n = numberOfCharacters;
  return n < 0 ? str : n >= str.length ? '' : str.slice(0, numberOfCharacters * -1);
};
...
const extractVowels = (str) => {
  ...
  const vowels = str
    ...
    .sort()
    .join('');
  return [str.replaceAll(/[aeiou]/g, ''), vowels];
