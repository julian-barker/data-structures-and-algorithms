...
const screenForNames = (arr) => {
  // Solution code here...
  const re = /^(Mr.|Mrs.|Ms.|Dr.) [a-zA-Z]+/;
  return arr.filter(v => re.test(v));
};
...
const toTitleCase = (arr) => {
  ...
  return arr.map(v => {
    const a = [...v];
    const title = a[0].toUpperCase().concat(a.slice(1).join(''));
    return title;
  });
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const newArr = arr.filter(v => parseInt(v.mass) > parseInt(arr[0].mass))
    .map(v => v.name);
  return newArr.join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  const arr = [...board];
  const arr2 = arr.concat(arr.reduce((acc, v) => {
    return v.map((val,i) => acc[i].concat([val]));
  }, [[],[],[]]))
    .concat([[arr[0][0],arr[1][1],arr[2][2]]])
    .concat([[arr[0][2],arr[1][1],arr[2][0]]]);
  console.log(arr2);
  return arr2.some(v => v.every(val => val === 'X') || v.every(val => val === 'O'));
};
