...
const containsAnd = (arr) => {
  // Solution code here...
  const re = /and/i;
  return arr.filter(str => str.match(re));
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  return arr.length === 0 ? arr : arr.filter(el => !forbiddenValues.includes(el) );
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter(v => v.baseStat > minBaseStat).map(v => v.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  return arr.filter(v => !Object.keys(v).includes('children') );
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter(v => typeof v === 'number' ).map(v => v % 2 === 0 ? 'even' : 'odd' );
};
